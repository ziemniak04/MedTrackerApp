...
class MedicationModelTests(TestCase):
    ...
    def test_adherence_rate_no_logs(self):
        med = Medication.objects.create(
            name="Empty", dosage_mg=10, prescribed_per_day=1
        )
    ...
    def test_adherence_rate_over_period_valid(self):
        med = Medication.objects.create(
            name="PeriodTest", dosage_mg=10, prescribed_per_day=1
        )
        start_date = timezone.now().date() - timedelta(days=2)
        end_date = timezone.now().date()

        DoseLog.objects.create(
            medication=med, taken_at=timezone.now() - timedelta(days=1), was_taken=True
        )
        DoseLog.objects.create(
            medication=med, taken_at=timezone.now() - timedelta(days=2), was_taken=False
        )
    ...
    def test_fetch_external_info_failure(self, mock_get_info):
        ...
        med = Medication.objects.create(
            name="TestMed", dosage_mg=10, prescribed_per_day=1
        )
